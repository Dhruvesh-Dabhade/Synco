...
object PacketCodec {
    ...
    fun serialize(packet: Packet): ProtocolResult<String> {
        return try {
            ...
            ProtocolResult.Failure(ProtocolError.SerializationFailure("Failed to serialize packet", e))
        } catch (e: IllegalArgumentException) {
            ProtocolResult.Failure(ProtocolError.SerializationFailure("Failed to serialize packet due to invalid arguments", e))
        }
    }
    ...
    fun deserialize(jsonString: String): ProtocolResult<Packet> {
        return try {
            ...
            ProtocolResult.Failure(ProtocolError.MalformedPacket("Failed to deserialize packet: ${e.message}"))
        } catch (e: IllegalArgumentException) {
            ProtocolResult.Failure(ProtocolError.MalformedPacket("Failed to deserialize packet due to invalid arguments: ${e.message}"))
        }
    }
    ...
    fun encryptPayload(packet: Packet, cryptoManager: CryptoManager): ProtocolResult<Packet> {
        ...
        return try {
            ...
            val (ciphertext, nonce) = cryptoManager.encrypt(payloadJson.toByteArray(Charsets.UTF_8))
            ...
            ProtocolResult.Failure(ProtocolError.SerializationFailure("Failed to encrypt payload", e))
        }
    }
    ...
    fun decryptPayload(packet: Packet, cryptoManager: CryptoManager): ProtocolResult<Packet> {
        ...
        return try {
            ...
            val plaintextBytes = cryptoManager.decrypt(ciphertext, nonce)
            ...
            ProtocolResult.Failure(ProtocolError.MalformedPacket("Nonce reuse or invalid argument: ${e.message}"))
        } catch (e: Exception) {
            ProtocolResult.Failure(ProtocolError.MalformedPacket("Failed to decrypt payload: ${e.message}"))
        }
    }
}
